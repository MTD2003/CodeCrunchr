...
from ..models.leaderboards import (
    LeaderboardRanking,
    LeaderboardResponse,
    WakatimeProfile,
)
...
async def get_leaderboard(
    ...
    current_week_start = date.fromisocalendar(
        year=today.year, week=today.isocalendar().week, day=1
    )
    ...
    async with get_session() as session:
        stmt = (
            ...
            .join(
                WakatimeUserProfile,
                WakatimeUserProfile.user_id == WeeklyLeaderboard.user_id,
                isouter=True,
            )
        )
...
async def get_leaderboard_placement_for_current_user(
    ...
    weekly leaderboard
    """
    return await get_leaderboard_placement_for_user(_=user_id, user_id=user_id)


@router.get("/leaderboard/placement/{user_id}")
async def get_leaderboard_placement_for_user(
    _: UserIDDependencyType, user_id: UUID
) -> LeaderboardRanking:
